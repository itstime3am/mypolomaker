...
function _fncDoPrepareInsertDeliveryOrder() {
	...
	for (var _i=0;_i<_arr_obj.length;_i++) {
		...
		var _percent_discount = ('percent_discount' in _ea) ? parseFloat(_ea['percent_discount']) : 0;
		var _each_discount = ('each_discount' in _ea) ? parseFloat(_ea['each_discount']) : 0;
		...
		var _str = '<tr rowid="-1" type_id="' + _type_id + '" order_rowid="' + _order_rowid + '" order_detail_rowid="' + _order_detail_rowid + '" left_qty="' + _left_qty + '" avg_deposit_payment="' + _avg_deposit_payment + '" avg_close_payment="' + _avg_close_payment + '" percent_discount="' + _percent_discount + '" each_discount="' + _each_discount + '" >';
		_str += '<td>' + _deliver_qty + '</td>';
		_str += '<td>' + _description + '</td>';
		_str += '<td>' + formatNumber(_total_price_each, 2) + '</td>';
		_str += '<td>' + _percent_discount + '%</td>';
		_str += '<td>' + formatNumber((_total_price_each - _each_discount) * _deliver_qty, 2) + '</td>';
