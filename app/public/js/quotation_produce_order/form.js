...
$(function() {
	...
	$('#txt-create_order_date, #txt-create_due_date, #txt-create_deliver_date, #txt-ws_sample_date').datepicker({
			showOn: "both",
...
function fnc__DDT_Row_RenderProduceOrderControl(data, type, full) {
	...
	if (_order_rowid > 0) {
		...
		switch (_type_id) {
			...
			case 11:
			case 15:
				_href = './order_other/pass_command/1/' + _order_rowid + '/' + _order_job_number;
				break;
			case 12:
			case 14:
				_href = './order_premade_other/pass_command/1/' + _order_rowid + '/' + _order_job_number;
				break;
...
function customCommand(command, aData, tr, divEditDlg) {
	...
	if ((_cmd == 'produce') && ('rowid' in aData)) {
		...
		if (confirm('สร้างใบงานสั่งผลิต ตามรายละเอียดลำดับที่ #' + _seq + ', ของใบเสนอราคาเลฃที่ "' + _qo_number + '", กรุณากด "ยืนยัน" เพื่อทำรายการ')) {
			...
			var _ws_sample_date = new Date();
			_ws_sample_date.setDate(_ws_sample_date.getDate() + 4);
			setValue($('#txt-ws_sample_date', _dlg), _ws_sample_date);
			_dlg.dialog("open");
		}
	}
}
