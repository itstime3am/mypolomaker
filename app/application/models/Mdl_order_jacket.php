...
class Mdl_order_jacket extends MY_Model {
	function __construct() {
		parent::__construct();
		if (! isset($this->db)) {
			$this->db = $this->load->database('default', TRUE);
		}
		$this->_TABLE_NAME = 't_order_jacket';
...
TO_CHAR(t.deliver_date, 'DD/MM/YYYY') AS disp_deliver_date, t.is_vat, t.has_sample, t.pattern_rowid, 
t.file_image1, t.file_image2, t.remark1, t.remark2, d.fabric_rowid, 
d.color, d.color_add1, d.color_add2, d.collar_detail, d.sleeves_detail, d.clasper_detail, 
d.remark1 AS detail_remark1, d.remark2 AS detail_remark2 
...
FROM t_order_jacket t
	INNER JOIN v_order_report_status v ON v.type_id = 6 AND v.order_rowid = t.rowid 
	INNER JOIN t_order_detail_jacket d ON t.rowid = d.order_rowid 
...
FROM pm_m_order_size s
	INNER JOIN pm_m_order_size_cat c ON s.main_cat_rowid = c.rowid 
	INNER JOIN pm_m_order_size_sub_cat sc ON s.sub_cat_rowid = sc.rowid 
	LEFT OUTER JOIN (SELECT * FROM t_order_size_jacket WHERE order_rowid = $RowID) t ON t.order_size_rowid = s.rowid 
WHERE s.is_jacket = 1 
