<?php
class Mdl_order_tshirt extends MY_Model {
	function __construct()
	{
		...
		$this->_TABLE_NAME = 'pm_t_order_tshirt';
...
, TO_CHAR(t.deliver_date, 'DD/MM/YYYY') AS disp_deliver_date, t.is_vat, t.has_sample, t.tshirt_pattern_rowid
, d.m_pen, d.f_pen, d.pen_pattern_rowid, t.size_category, t.file_image1, t.file_image2, t.remark1, t.remark2
, d.neck_type_rowid, d.neck_type_detail, d.standard_pattern_rowid, d.standard_pattern_detail, d.fabric_rowid
, d.color, d.color_add1, d.color_add2, d.collar_type_rowid, d.collar_detail, d.sleeves_type_rowid, d.sleeves_detail
, d.flap_type_rowid, d.flap_type_detail, d.m_flap_side, d.f_flap_side, d.flap_side_ptrn_rowid, d.flap_side_ptrn_detail
, d.m_pocket, d.f_pocket, d.pocket_type_rowid, d.pocket_type_detail, d.remark1 AS detail_remark1, d.remark2 AS detail_remark2
...
FROM pm_t_order_tshirt t 
	INNER JOIN v_order_report_status v ON v.type_id = 2 AND v.order_rowid = t.rowid 
	INNER JOIN pm_t_order_detail_tshirt d ON t.rowid = d.order_rowid 
...
WHERE s.is_tshirt = 1 
-- AND s.is_use = 1 
ORDER BY CASE WHEN c.rowid = 8 THEN 0 ELSE c.rowid END, c.rowid, CASE sc.rowid WHEN 3 THEN 1 WHEN 2 THEN 2 WHEN 1 THEN 3 ELSE sc.rowid END, s.rowid
EOT;
	...
	function delete($RowID) {
		...
		if ($_rowid > 0) {
			while ($this->error_message == '') {
				$this->db->delete('pm_t_order_detail_tshirt', array('order_rowid'=>$_rowid));
				...
				$this->db->delete('t_order_add_option', array('order_type_id'=>2, 'order_rowid'=>$_rowid));
				if ($this->db->error()['message'] !== "") {
					$this->error_message = $this->db->error()['message'];
					break;
				}
...
EOT;
		...
		while ($_whileChecker) {
			...
			$_sql = <<<EOT
SELECT s.size_text, s.size_chest, COALESCE(t.qty, 0) AS qty, TO_CHAR(COALESCE(t.price, 0), '9G999G990D00') AS price
, CASE sc.rowid WHEN 1 THEN 'c' WHEN 2 THEN 'f' WHEN 3 THEN 'm' WHEN 4 THEN 'c' ELSE '-' END AS type 
FROM pm_m_order_size s
INNER JOIN pm_m_order_size_cat c ON s.main_cat_rowid = c.rowid 
INNER JOIN pm_m_order_size_sub_cat sc ON s.sub_cat_rowid = sc.rowid 
LEFT OUTER JOIN (SELECT * FROM pm_t_order_size_tshirt WHERE order_rowid = $RowID) AS t ON t.order_size_rowid = s.rowid 
WHERE s.is_tshirt = 1 
AND (s.is_use = 1 OR COALESCE(t.qty, 0) > 0)
AND (c.rowid = $_size_cat OR $_size_cat = -1) 
...
EOT;
			...
			$_sql = <<<EOT
SELECT size_text, size_chest, qty, TO_CHAR(COALESCE(price, 0), '9G999G990D00') AS price
, CASE sub_cat_rowid WHEN 1 THEN 'c' WHEN 2 THEN 'f' WHEN 3 THEN 'm' WHEN 4 THEN 'c' ELSE '-' END AS type  
FROM pm_t_order_size_custom_tshirt 
WHERE order_rowid = $RowID 
AND (main_cat_rowid = $_size_cat OR $_size_cat = -1) 
...
SEL;
		...
		if (($this->m->error_message == '') && (count($_item) > 0)) {
			return $_item;
		} else {
			return FALSE;
		}
