<?php
class Mdl_order_premade_polo extends MY_Model {
	function __construct()
	{
		...
		$this->_TABLE_NAME = 'pm_t_order_premade_polo';
...
, TO_CHAR(t.deliver_date, 'DD/MM/YYYY') AS disp_deliver_date, t.is_vat, t.has_sample, t.file_image1, t.file_image2, t.remark1, t.remark2 
...
FROM pm_t_order_premade_detail_polo AS d
	INNER JOIN pm_t_polo_pattern AS p ON d.pattern_rowid = p.rowid 
...
SELECT s.* 
FROM pm_t_order_premade_detail_polo AS d 
	INNER JOIN pm_t_order_premade_size_polo AS s ON d.rowid = s.order_detail_rowid
WHERE d.order_rowid = $RowID
EOT;
...
FROM pm_t_order_premade_price_polo t 
...
FROM pm_t_order_premade_polo m
	INNER JOIN v_order v ON v.type_id = 3 AND v.order_rowid = m.rowid
...
FROM pm_m_premade_order_size s
	INNER JOIN pm_m_order_size_sub_cat sc ON s.sub_cat_rowid = sc.rowid
	INNER JOIN (
			SELECT sz.main_cat_rowid, sz.sub_cat_rowid
			FROM pm_t_order_premade_size_polo s
				INNER JOIN pm_t_order_premade_detail_polo d ON d.rowid = s.order_detail_rowid
				INNER JOIN pm_m_premade_order_size sz ON sz.rowid = s.order_size_rowid
			WHERE d.order_rowid = $RowID
			GROUP BY sz.main_cat_rowid, sz.sub_cat_rowid
	) ft ON ft.sub_cat_rowid = s.sub_cat_rowid
WHERE s.is_polo > 0
ORDER BY CASE sc.rowid WHEN 3 THEN 1 WHEN 2 THEN 2 WHEN 1 THEN 3 ELSE sc.rowid END, s.sort_index
TMPL;
			$_arr = $this->arr_execute($_sql);
			$_arrIndex = array();
			if ($_arr !== FALSE) {
...
FROM (
	...
	INNER JOIN pm_t_polo_pattern AS p ON _sq.pattern_rowid = p.rowid
EDT;
			...
			$_sql .= 'FROM pm_t_order_premade_price_polo t WHERE t.order_rowid = ' . $RowID . ' ORDER BY t.seq';
			$_arr3 = $this->m->arr_execute($_sql);
			$_strError = $this->m->error_message;
			if ($_strError != '') {
				$_whileChecker = FALSE;
...
EOT;
	...
	function change_status_by_id($detail_rowid, $ps_rowid, $status_remark = FALSE) {
		...
		$this->db->update('pm_t_order_premade_detail_polo');
