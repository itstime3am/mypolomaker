...
class Mdl_order_premade_jacket extends MY_Model {
	function __construct() {
		parent::__construct();
		if (! isset($this->db)) {
			$this->db = $this->load->database('default', TRUE);
		}
		$this->_TABLE_NAME = 't_order_premade_jacket';
...
FROM {$this->_TABLE_NAME} t 
	INNER JOIN v_order_report_status v ON v.type_id = 8 AND v.order_rowid = t.rowid 
...
EOT;
	...
	function list_size_quan() {
		$_sql = <<<EOT
SELECT s.rowid, c.rowid AS cat_rowid, COALESCE(c.name, '') AS category, sc.rowid as sub_rowid, COALESCE(sc.name, '') AS sub_category
, s.size_text, s.size_chest
, CASE WHEN s.is_use > 0 THEN 0 ELSE 1 END AS is_expired
FROM pm_m_premade_order_size s
	LEFT OUTER JOIN pm_m_order_size_cat c ON s.main_cat_rowid = c.rowid 
	LEFT OUTER JOIN pm_m_order_size_sub_cat sc ON s.sub_cat_rowid = sc.rowid 
WHERE s.is_jacket = 1 
-- AND s.is_use = 1 
ORDER BY c.rowid, CASE sc.rowid WHEN 3 THEN 1 WHEN 2 THEN 2 WHEN 1 THEN 3 ELSE sc.rowid END, s.rowid
EOT;
...
FROM t_order_premade_detail_jacket AS d
	INNER JOIN t_jacket_pattern AS p ON d.pattern_rowid = p.rowid 
...
SELECT s.* 
FROM t_order_premade_detail_jacket AS d 
	INNER JOIN t_order_premade_size_jacket AS s ON d.rowid = s.order_detail_rowid
WHERE d.order_rowid = $RowID
EOT;
...
FROM t_order_premade_price_jacket t 
...
FROM t_order_premade_jacket m
	INNER JOIN v_order v ON v.type_id = 8 AND v.order_rowid = m.rowid
...
FROM pm_m_premade_order_size s
	...
	INNER JOIN (
			...
			FROM t_order_premade_size_jacket s
				INNER JOIN t_order_premade_detail_jacket d ON d.rowid = s.order_detail_rowid
...
WHERE s.is_jacket > 0
ORDER BY CASE sc.rowid WHEN 3 THEN 1 WHEN 2 THEN 2 WHEN 1 THEN 3 ELSE sc.rowid END, s.rowid
TMPL;
			$_arr = $this->arr_execute($_sql);
			$_arrIndex = array();
			if ($_arr !== FALSE) {
...
FROM (
	...
	INNER JOIN t_jacket_pattern AS p ON _sq.pattern_rowid = p.rowid
EDT;
...
FROM t_order_premade_screen_jacket t 
...
EOT;
	...
	function change_status_by_id($detail_rowid, $ps_rowid, $status_remark = FALSE) {
		...
		$this->db->update('t_order_premade_detail_jacket');
