...
class Order_jacket extends MY_Ctrl_crud {
	function __construct() {
		parent::__construct();
		$this->modelName = 'Mdl_order_jacket';
	...
	public function index($customer_rowid = -1) {
		...
		$this->_selOptions = $this->_prepareSelectOptions(array(
				'standard_pattern'=>array('where'=>array('is_cancel'=>0,'is_jacket'=>1),'order_by'=>'sort_index')
		...
		$_temp = $this->p->search();
		$this->_selOptions['jacket_pattern'] = array(array('rowid'=>'-1', 'code'=>'- กำหนดเอง -'));
		...
		$_editFormParams['title'] = 'สั่งตัด : เสื้อแจ๊คเก็ต';
		...
		$_editFormParams['size_quan_panel'] = $this->add_view('order/_size_quan', array(
				'size_quan_matrix' => $this->$_mdl->list_size_quan() // "ขนาดเสื้อสำเร็จรูปแจ็คเก็ต" //
			), TRUE
		);
		//-- size_quan panel form parts
		...
		$_editFormParams['screen_panel'] = $this->add_view('order/_screen', array(
				...
				,'arr_position_list' => array('','อกซ้าย','อกขวา','แขนซ้าย','แขนขวา','ในสาบหลัง','ใต้สาบหลัง','กลางหลัง')
			), TRUE
		);
		//-- screen panel form parts
		...
		$_editFormParams['pattern_list'] = $this->_selOptions['jacket_pattern'];
		...
		$_editFormParams['crud_controller'] = 'order_jacket';
		...
		*/
		...
		if ($this->_blnCheckRight('export_pdf')) {
			if ($this->_blnCheckRight('edit', 'order')) {
				$_custom_columns[] = array(
					"column" => '{"sTitle":"ใบงาน", "sClass": "center","mData": function() { return \'<img class="tblButton" command="pdf_1" src="./public/images/pdf_icon_40.png" title="ใบสั่งซื้อแจ็คเก็ตสั่งตัด" /><img class="tblButton" command="pdf_6" src="./public/images/pdf_icon_40.png" title="ใบงานข้อมูล แจ็คเก็ตสั่งตัด" />\';}, "bSortable": false}'
					, "order" => 15
				);
			} else {
				$_custom_columns[] = array(
		...
		$pass['title'] = "สั่งตัดเสื้อแจ็คเก็ต";
	...
	function __getDetailsPanel() {
		...
		$_temp = hlpr_prepareControlsDefault('Mdl_order_detail_jacket', $this->_selOptions);
		$_details = array();
		...
		return $this->add_view('order/_detail', array(
...
/*
				// ++ size quantity !! this should always none since jacket have only 1 orer_size category and treat as CUSTOM!
				$_sql =<<<SQL

SELECT order_size_rowid, qty, TO_CHAR(COALESCE(price, 0), '9G999G990D00') AS price 
FROM t_order_size_jacket 
...
FROM t_order_size_custom_jacket 
WHERE order_rowid = $_rowid 
