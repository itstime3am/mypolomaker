...
class Order_tshirt extends MY_Ctrl_crud {
	function __construct() {
		parent::__construct();
		$this->modelName = 'Mdl_order_tshirt';
	...
	public function index($customer_rowid = -1) {
		...
		$this->load->model('Mdl_tshirt_pattern', 'p');
		$_temp = $this->p->search();
		$this->_selOptions['tshirt_pattern'] = array(array('rowid'=>'-1', 'code'=>'- custom -'));
		foreach ($_temp as $_row) {
			...
			array_push($this->_selOptions['tshirt_pattern'], $_each);
		}

		$_editFormParams['details_panel'] = $this->__getDetailsPanel();
		$_editFormParams['title'] = 'สั่งตัด : เสื้อยืด';
		...
		$_editFormParams['pattern_list'] = $this->_selOptions['tshirt_pattern'];
		$_editFormParams['pen_pattern_list'] = $this->_selOptions['pen_pattern'];
		//$_editFormParams['pen_position_list'] = $this->_selOptions['pen_position'];
		...
		$_editFormParams['crud_controller'] = 'order_tshirt';
		...
		*/
		...
		if ($this->_blnCheckRight('export_pdf')) {
			if ($this->_blnCheckRight('edit', 'order')) {
				$_custom_columns[] = array(
					"column" => '{"sTitle":"เอกสาร", "sClass": "center","mData": function() { return \'<img class="tblButton" command="pdf_1" src="./public/images/pdf_icon_40.png" title="FM-SA-02-001: ใบสั่งซื้อเสื้อยืดสั่งตัด" /><img class="tblButton" command="pdf_2" src="./public/images/pdf_icon_40.png" title="FM-SA-02-002: ใบสั่งตัดเสื้อยืด" /><img class="tblButton" command="pdf_3" src="./public/images/pdf_icon_40.png" title="FM-SA-02-003: ใบจ่ายงานเย็บเสื้อยืด" /><img class="tblButton" command="pdf_6" src="./public/images/pdf_icon_40.png" title="FM-SA-02-004: ใบงานข้อมูล เสื้อยืด" />\';}, "bSortable": false}'
					, "order" => 15
				);
			} else {
				$_custom_columns[] = array(
					"column" => '{"sTitle":"เอกสาร", "sClass": "center","mData": function() { return \'<img class="tblButton" command="pdf_1" src="./public/images/pdf_icon_40.png" title="FM-SA-02-001: ใบสั่งซื้อเสื้อยืดสั่งตัด" /><img class="tblButton" command="pdf_6" src="./public/images/pdf_icon_40.png" title="FM-SA-02-004: ใบงานข้อมูล เสื้อยืด" />\';}, "bSortable": false}'
					, "order" => 15
				);
			}
		}
		...
		$pass['work_panel'] = $this->add_view('_public/_list', 
			array(
				...
				'edit_template' => $this->load->view('order/form_tshirt', $_editFormParams, TRUE)
			), TRUE
		);
	...
	function __getDetailsPanel() {
		...
		$_temp = hlpr_prepareControlsDefault('Mdl_order_detail_tshirt', $this->_selOptions);
		$_details = array();
		...
		hlpr_setController($_details, 'm_collar_type_rowid', 'แบบซกคอชาย', array('sel_options'=>$this->_selOptions['collar_type'])); //new
		hlpr_setController($_details, 'f_collar_type_rowid', 'แบบซกคอหญิง', array('sel_options'=>$this->_selOptions['collar_type'])); //new
		...
		hlpr_setController($_details, 'm_sleeves_type_rowid', 'แขนเสื้อชาย', array('sel_options'=>$this->_selOptions['sleeves_type'])); //- new 
		hlpr_setController($_details, 'f_sleeves_type_rowid', 'แขนเสื้อหญิง', array('sel_options'=>$this->_selOptions['sleeves_type'])); //- new 
	...
	function get_order_by_id() {
		...
		if (($_rowid != FALSE) && $_rowid > '0') {
			...
			while ($_whileChecker) {
				...
				// ++ size quantity
				$_sql = "SELECT order_size_rowid, qty, TO_CHAR(COALESCE(price, 0), '9G999G990D00') AS price FROM pm_t_order_size_tshirt WHERE order_rowid = " . $_rowid . " ORDER BY seq";
				...
				$_sql = "SELECT main_cat_rowid, sub_cat_rowid, size_text, size_chest, qty, TO_CHAR(COALESCE(price, 0), '9G999G990D00') AS price FROM pm_t_order_size_custom_tshirt WHERE order_rowid = " . $_rowid . " ORDER BY seq, sub_cat_rowid";
...
FROM pm_t_order_price_tshirt t 
...
OTP;
				...
				tmp.fabric_date , tmp.block_emp, tmp.approve_date, d.seq, d.order_rowid, tmp.prod_status, tmp.status_remark
				, ARRAY_TO_JSON(ARRAY(
		...
		if (isset($_arrData) && ($_arrData != FALSE)) {
			try {
				...
				if (array_key_exists('job_number', $_arr)) {
					...
					$_exists = $this->_mas->int_exists_job_number($_arr['job_number'], 2, (isset($_arr['rowid']) ? $_arr['rowid'] : -1));
					if ($_exists > 0) {
				...
				while ($_strError == '') {
					...
					$_aopRowid = -1;
					$_rslt = $this->db->query('SELECT rowid FROM t_order_add_option WHERE order_type_id = 2 AND order_rowid = ?', array($_rowid));
					if ($_rslt->num_rows > 0) {
						$_aopRowid = $_rslt->first_row()->rowid;
					}
					$this->load->model('Mdl_order_add_option', 'aopt');
					// ++ order detail
					$this->load->model('Mdl_order_detail_tshirt', 'md');
					...
					$this->aopt->_FIELDS['order_type_id'] = 2; //tshirt type
					...
					// ++ Size quantity
					$this->db->delete('pm_t_order_size_tshirt', array('order_rowid'=>$_rowid));
					$_bulk = array();
					if (array_key_exists('size', $_arr)) {
						if (is_array($_arr['size'])) {
							...
							if (count($_bulk) > 0) {
								$this->db->insert_batch('pm_t_order_size_tshirt', $_bulk);
							}
						}
					}
					...
					// ++ Custom size quantity
					$this->db->delete('pm_t_order_size_custom_tshirt', array('order_rowid'=>$_rowid));
					$_bulk = array();
					if (array_key_exists('size_custom', $_arr)) {
						if (is_array($_arr['size_custom'])) {
							foreach ($_arr['size_custom'] as $_cat_rowid => $_cat_arr) {
								...
								foreach ($_cat_arr as $_sub_rowid => $_sub_arr) {
									...
									foreach ($_sub_arr as $_cus_index => $_cus_obj) {
										...
										$_cus_qty = 0;
										$_cus_price = 0;
										if (array_key_exists('qty', $_cus_obj) && (intval($_cus_obj['qty']) > 0)) {
											...
											array_push($_bulk, 
												array(
													...
													, 'qty' => $_cus_qty
													, 'price' => $_cus_price
													, 'seq' => ((intval($_cus_index) > 0) ? intval($_cus_index) : NULL)
												)
											);
										}
									}
							...
							if (count($_bulk) > 0) {
								$this->db->insert_batch('pm_t_order_size_custom_tshirt', $_bulk);
							}
						}
					}
					if ($this->db->_error_message() !== "") {
					...
					// ++ Others price
					$this->db->delete('pm_t_order_price_tshirt', array('order_rowid'=>$_rowid));
					$_bulk = array();
					if (array_key_exists('others_price', $_arr)) {
						if (is_array($_arr['others_price'])) {
							...
							if (count($_bulk) > 0) {
								$this->db->insert_batch('pm_t_order_price_tshirt', $_bulk);
							}
						}
					}
					if ($this->db->_error_message() !== "") {
					...
					// ++ Screen
					$this->db->delete('pm_t_order_screen_tshirt', array('order_rowid'=>$_rowid));
					$_bulk = array();
					if (array_key_exists('screen', $_arr)) {
						if (is_array($_arr['screen'])) {
							...
							if (count($_bulk) > 0) {
								$this->db->insert_batch('pm_t_order_screen_tshirt', $_bulk);
							}
						}
					}
