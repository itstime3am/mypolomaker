...
class Order_premade_tshirt extends MY_Ctrl_crud {
	function __construct() {
		parent::__construct();
		$this->modelName = 'Mdl_order_premade_tshirt';
	...
	public function index($customer_rowid = -1) {
		...
		$this->_selOptions = $this->_prepareSelectOptions(array(
			//'pen_pattern'=>array('where'=>array('is_tshirt'=>1)),
			//'pen_position'=>array('where'=>array('is_tshirt'=>1)),
			'order_screen'=>array('where'=>array('is_tshirt'=>1))
		...
		$_temp = $this->p->search();
		$this->_selOptions['tshirt_pattern'] = array(array('rowid'=>'', 'code'=>'', 'color'=>''));
		...
		$_editFormParams['details_panel'] = $this->add_view('order/_detail_premade', array(
			...
			'pattern_list' => $this->_selOptions['tshirt_pattern']
		), TRUE);
		//-- details panel
		...
		$_editFormParams['crud_controller'] = 'order_premade_tshirt';
		...
		*/
		...
		if ($this->_blnCheckRight('export_pdf')) $_custom_columns[] = array(
				"column" => '{"sTitle":"ใบงาน", "sClass": "center", "mData": function() { return \'<img class="tblButton" command="pdf_1" src="./public/images/pdf_icon_40.png" title="ใบสั่งซื้อเสื้อยืดสำเร็จรูป" /><img class="tblButton" command="pdf_6" src="./public/images/pdf_icon_40.png" title="ใบงานข้อมูล เสื้อยืดสำเร็จรูป" />\';}, "bSortable": false}'
				, "order" => 15
			);
		...
		$pass['title'] = "สั่งสำเร็จรูปเสื้อยืด";
...
OSL
		...
		if (isset($_arrData) && ($_arrData != FALSE)) {
			try {
				...
				if (array_key_exists('job_number', $_arr)) {
					...
					$_exists = $this->_mas->int_exists_job_number($_arr['job_number'], 4, (isset($_arr['rowid']) ? $_arr['rowid'] : -1));
					if ($_exists > 0) {
				...
				while ($_strError == '') {
					...
					//++ details size
					$this->db->query('delete from pm_t_order_premade_size_tshirt where order_detail_rowid in (select rowid from pm_t_order_premade_detail_tshirt where order_rowid = ' . $_rowid . ')');
					
					$this->db->delete('pm_t_order_premade_detail_tshirt', array('order_rowid'=>$_rowid));
					$_bulkSize = array();
					if (array_key_exists('detail', $_arr)) {
						if (is_array($_arr['detail'])) {
							foreach ($_arr['detail'] as $_obj) {
								if (array_key_exists('pattern_rowid', $_obj) && array_key_exists('color', $_obj)) {
									if (intval($_obj['pattern_rowid']) > 0) {
										...
										$this->db->insert('pm_t_order_premade_detail_tshirt');
										$_strError = $this->db->_error_message();
									
										$_avg_price = 0;
										if (array_key_exists('price', $_obj) && (is_numeric($_obj['price']))) $_avg_price = (float) $_obj['price'];
										$_detail_rowid = 0;
										if ($_strError == '') {
												...
												if (is_array($_obj['order_size'])) {
													foreach ($_obj['order_size'] as $_size) {
														if (array_key_exists('order_size_rowid', $_size) && array_key_exists('qty', $_size)) {
															if ((intval($_size['order_size_rowid']) > 0) && (intval($_size['qty']) > 0)) {
																array_push($_bulkSize, 
																	array(
																		'order_detail_rowid'=> $_detail_rowid
																		, 'order_size_rowid'=> (int) $_size['order_size_rowid']
																		, 'qty'=> (int) $_size['qty']
																		, 'price'=> (double) $_avg_price
																	)
																);
															}
														}
							...
							if (count($_bulkSize) > 0) {
								$this->db->insert_batch('pm_t_order_premade_size_tshirt', $_bulkSize);
							}
						}
					}
					...
					// ++ Others price
					$this->db->delete('pm_t_order_premade_price_tshirt', array('order_rowid'=>$_rowid));
					$_bulk = array();
					if (array_key_exists('others_price', $_arr)) {
						if (is_array($_arr['others_price'])) {
							...
							if (count($_bulk) > 0) {
								$this->db->insert_batch('pm_t_order_premade_price_tshirt', $_bulk);
							}
						}
					}
					if ($this->db->_error_message() !== "") {
					...
					//++ screen
					$this->db->delete('pm_t_order_premade_screen_tshirt', array('order_rowid'=>$_rowid));
					$_bulk = array();
					if (array_key_exists('screen', $_arr)) {
						if (is_array($_arr['screen'])) {
							...
							if (count($_bulk) > 0) {
								$this->db->insert_batch('pm_t_order_premade_screen_tshirt', $_bulk);
							}
						}
					}
					if ($this->db->_error_message() !== "") {
	...
	function get_pdf($pdf_index, $rowid) {
		...
		if ($pass['data'] == FALSE) {
			...
			switch ($pdf_index) {
				case "1":
					$file_name = 'FM-SA-04-001_' . $strNow . '.pdf';
					$pass['title'] = 'ใบสั่งซื้อ เสื้อยืดสำเร็จรูป';
					$pass['code'] = sprintf('FM-SA-04-001 REV.00 / %02d', $_rev_no);
					...
					break;
				case "6":
					$file_name = 'FM-SA-04-002_' . $strNow . '.pdf';
					$pass['title'] = 'ใบงานข้อมูล เสื้อยืดสำเร็จรูป';
					$pass['code'] = sprintf('FM-SA-04-002 REV.00 / %02d', $_rev_no);
