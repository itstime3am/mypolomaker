...
class Master_pen_type extends MY_Ctrl_crud {
	function __construct() 
	{
		parent::__construct();
		$this->modelName = 'Mdl_pen_type';
	}
	...
	public function index()
	{
		...
		$this->_setController("name", "ชนิด", NULL, array("selectable"=>TRUE, "width"=>"300px", "class"=>"center", "default"=>TRUE,"order"=>0));		
		$_arr_custom_columns = array(
			array(
				"column" => '{"sTitle": "สำหรับเสื้อโปโล", "sClass": "center", "mData": "is_shirt", "mRender": function(data,type,full) { if (full.is_shirt == 1) { return \'<img src="public/images/checkbox_yes.png" class="cls-is-true">\'; } else { return ""; } }, "orderable": true, "targets": "is_shirt"}'
				, "order" => 1
			)
			, array(
		...
		$this->_setController("is_shirt", "เสื้อโปโล", array("type"=>"chk"));
		...
		$pass['left_panel'] = $this->add_view(
			...
			, array(
				'controls' => array(
					...
					, array("type" => "chk", "label" => "เสื้อโปโล", "name" => "is_shirt") /*, "value" => TRUE*/
					, array("type" => "chk", "label" => "เสื้อยืด", "name" => "is_tshirt") /*, "value" => TRUE*/
				)
				, "layout" => array(
					array("return &nbsp;")
					, array("name")
					, array("is_shirt", "is_tshirt")
				)
			)
			, TRUE
		);
