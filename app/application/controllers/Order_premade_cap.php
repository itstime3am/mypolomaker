...
class Order_premade_cap extends MY_Ctrl_crud {
	function __construct() {
		parent::__construct();
		$this->modelName = 'Mdl_order_premade_cap';
	...
	public function index($customer_rowid = -1) {
		...
		$this->add_js(array(
			...
			'public/js/order/_detail_premade.js'
		));
...
*/		
		...
		$_temp = $this->p->search();
		$this->_selOptions['cap_pattern'] = array(array('rowid'=>'', 'code'=>'', 'color'=>''));
		...
		$_editFormParams['details_panel'] = $this->add_view('order/_detail_premade_single_size', array('pattern_list' => $this->_selOptions['cap_pattern']), TRUE);
		//-- details panel
		...
		$_editFormParams['crud_controller'] = 'order_premade_cap';
		...
		$_custom_columns[] = array(
				"column" => '{"sTitle":"ขำระมัดจำ", "sClass":"cls-payment-dlg right","sWidth":"80px","mData":"rowid","mRender": function(data,type,full) { return \'<span class="cls-spn-pament">\' + formatNumber(full.deposit_payment) + \'</span>\';}, "bSortable": true}'
				, "order" => 7
			);
		$this->_setController("left_amount", "คงเหลือ(บาท)", NULL, array("selectable"=>TRUE,"default"=>TRUE,"class"=>"default_number","order"=>8));
		/*
		$_custom_columns[] = array(
				"column" => '{"sTitle":"รายการชำระ", "sClass":"cls-payment-dlg right","sWidth":"80px","mData":"rowid","mRender": function(data,type,full) { return \'<span class="cls-spn-payment">\' + formatNumber(full.total_payment) + \'</span><img class="tblButton" command="cmd_open_payment_dialog" src="public/images/forms.png" title="รายการชำระเงิน" />\';}, "bSortable": true}'
				, "order" => 9
			);
		*/
		...
		if ($this->_blnCheckRight('export_pdf')) $_custom_columns[] = array(
				"column" => '{"sTitle":"ใบงาน", "sClass": "center","mData": function() { return \'<img class="tblButton" command="pdf_1" src="./public/images/pdf_icon_40.png" title="สั่งซื้อหมวกสำเร็จรูป" /><img class="tblButton" command="pdf_6" src="./public/images/pdf_icon_40.png" title="ใบงานข้อมูล หมวกสำเร็จรูป" />\';}, "bSortable": false}'
				, "order" => 15
			);
		$_custom_columns[] = array(
		...
		if (isset($_arrData) && ($_arrData != FALSE)) {
			try {
				...
				if (array_key_exists('job_number', $_arr)) {
					...
					$_exists = $this->_mas->int_exists_job_number($_arr['job_number'], 7, (isset($_arr['rowid']) ? $_arr['rowid'] : -1));
					if ($_exists > 0) {
				...
				while ($_strError == '') {
					...
					//++ details size
					$this->db->query('delete from t_order_premade_size_cap where order_detail_rowid in (select rowid from t_order_premade_detail_cap where order_rowid = ' . $_rowid . ')');
					$this->db->delete('t_order_premade_detail_cap', array('order_rowid'=>$_rowid));
					$_bulkSize = array();
					if (array_key_exists('detail', $_arr)) {
						if (is_array($_arr['detail'])) {
							foreach ($_arr['detail'] as $_obj) {
								if (array_key_exists('pattern_rowid', $_obj) && array_key_exists('color', $_obj)) {
									if (intval($_obj['pattern_rowid']) > 0) {
										...
										$this->db->insert('t_order_premade_detail_cap');
							...
							if (count($_bulkSize) > 0) {
								$this->db->insert_batch('t_order_premade_size_cap', $_bulkSize);
							}
						}
					}
					...
					// ++ Others price
					$this->db->delete('t_order_premade_price_cap', array('order_rowid'=>$_rowid));
					$_bulk = array();
					if (array_key_exists('others_price', $_arr)) {
						if (is_array($_arr['others_price'])) {
							...
							if (count($_bulk) > 0) {
								$this->db->insert_batch('t_order_premade_price_cap', $_bulk);
							}
						}
					}
					if ($this->db->_error_message() !== "") {
					...
					//++ screen
					$this->db->delete('t_order_premade_screen_cap', array('order_rowid'=>$_rowid));
					$_bulk = array();
					if (array_key_exists('screen', $_arr)) {
						if (is_array($_arr['screen'])) {
							...
							if (count($_bulk) > 0) {
								$this->db->insert_batch('t_order_premade_screen_cap', $_bulk);
							}
						}
					}
					if ($this->db->_error_message() !== "") {
