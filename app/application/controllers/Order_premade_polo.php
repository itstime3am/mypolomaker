...
class Order_premade_polo extends MY_Ctrl_crud {
	function __construct() {
		parent::__construct();
		$this->modelName = 'Mdl_order_premade_polo';
	...
	public function index($customer_rowid = -1) {
		...
		$this->_selOptions = $this->_prepareSelectOptions(array(
			'order_screen'=>array('where'=>array('is_polo'=>1))
		...
		$_temp = $this->p->search();
		$this->_selOptions['polo_pattern'] = array(array('rowid'=>'', 'code'=>'', 'color'=>''));
		...
		$_editFormParams['details_panel'] = $this->add_view('order/_detail_premade', array(
			...
			'pattern_list' => $this->_selOptions['polo_pattern']
		), TRUE);
		//-- details panel
		...
		//$_editFormParams['job_number_list'] = $this->m_model->list_job_number();
		...
		$_editFormParams['crud_controller'] = 'order_premade_polo';
		...
		*/
		...
		if ($this->_blnCheckRight('export_pdf')) $_custom_columns[] = array(
				"column" => '{"sTitle":"ใบงาน", "sClass": "center", "mData": function() { return \'<img class="tblButton" command="pdf_1" src="./public/images/pdf_icon_40.png" title="ใบสั่งซื้อเสื้อโปโลสำเร็จรูป" /><img class="tblButton" command="pdf_6" src="./public/images/pdf_icon_40.png" title="ใบงานข้อมูล เสื้อโปโลสำเร็จรูป" />\';}, "bSortable": false}'
				, "order" => 15
			);
		...
		$pass['title'] = "สั่งสำเร็จรูปเสื้อโปโล";
		...
		if (isset($_arrData) && ($_arrData != FALSE)) {
			try {
				...
				if (array_key_exists('job_number', $_arr)) {
					...
					$_exists = $this->_mas->int_exists_job_number($_arr['job_number'], 3, (isset($_arr['rowid']) ? $_arr['rowid'] : -1));
					if ($_exists > 0) {
				...
				while ($_strError == '') {
					...
					//++ details size
					$this->db->query('delete from pm_t_order_premade_size_polo where order_detail_rowid in (select rowid from pm_t_order_premade_detail_polo where order_rowid = ' . $_rowid . ')');
					
					$this->db->delete('pm_t_order_premade_detail_polo', array('order_rowid'=>$_rowid));
					$_bulkSize = array();
					if (array_key_exists('detail', $_arr)) {
						if (is_array($_arr['detail'])) {
							foreach ($_arr['detail'] as $_obj) {
								if (array_key_exists('pattern_rowid', $_obj) && array_key_exists('color', $_obj)) {
									if (intval($_obj['pattern_rowid']) > 0) {
										...
										$this->db->insert('pm_t_order_premade_detail_polo');
							...
							if (count($_bulkSize) > 0) {
								$this->db->insert_batch('pm_t_order_premade_size_polo', $_bulkSize);
							}
						}
					}
					...
					// ++ Others price
					$this->db->delete('pm_t_order_premade_price_polo', array('order_rowid'=>$_rowid));
					$_bulk = array();
					if (array_key_exists('others_price', $_arr)) {
						if (is_array($_arr['others_price'])) {
							...
							if (count($_bulk) > 0) {
								$this->db->insert_batch('pm_t_order_premade_price_polo', $_bulk);
							}
						}
					}
					if ($this->db->_error_message() !== "") {
					...
					//++ screen
					$this->db->delete('pm_t_order_premade_screen_polo', array('order_rowid'=>$_rowid));
					$_bulk = array();
					if (array_key_exists('screen', $_arr)) {
						if (is_array($_arr['screen'])) {
							...
							if (count($_bulk) > 0) {
								$this->db->insert_batch('pm_t_order_premade_screen_polo', $_bulk);
							}
						}
					}
					if ($this->db->_error_message() !== "") {
		...
		if ($pass['data'] == FALSE) {
			...
			switch ($pdf_index) {
				case "1":
					...
					$pass['title'] = 'ใบสั่งซื้อ เสื้อโปโลสำเร็จรูป';
