...
class Report_deliver extends MY_Ctrl_crud {
	...
	public function index($product_type_id = 0) {
		...
		$this->add_js(
			array(
				...
				'public/js/report/report_deliver.js',
		...
		$this->_setController("order_status", "สถานะ", array(), array("selectable"=>TRUE,"default"=>TRUE,"class"=>"center edit order_status","width"=>'80px',"order"=>0));
		...
		$this->_setController("category", "กลุ่มสินค้า", array(), array("selectable"=>TRUE,"class"=>"center","order"=>6));
		$this->_setController("type", "ประเภทสินค้า", array(), array("selectable"=>TRUE,"class"=>"center","order"=>7));
...
/*
		...
		$this->_setController("screen_status", "สถานะสกรีน", array(), array("selectable"=>TRUE,"default"=>TRUE,"class"=>"center edit screen_status","order"=>15));
		$this->_setController("is_weave", "ปัก", array(), array("selectable"=>TRUE,"default"=>TRUE,"class"=>"center","order"=>16));
		$this->_setController("weave_status", "สถานะปัก", array(), array("selectable"=>TRUE,"default"=>TRUE,"class"=>"center edit weave_status","order"=>17));
*/
		...
		$this->_setController("status_deliver_date", "วันที่ส่งจริง", array(), array("selectable"=>TRUE,"default"=>TRUE,"class"=>"center edit status_deliver_date","order"=>20));
		$this->_setController("sum_qty", "จำนวนตัว", array(), array("selectable"=>TRUE,"default"=>TRUE,"class"=>"right default_int","order"=>21));
/*
		$this->_setController("ea_item_price", "ราคาเสื้อเฉลี่ย", array(), array("selectable"=>TRUE,"default"=>TRUE,"class"=>"right","order"=>21));
		$this->_setController("ea_screen_price", "ราคาสกรีนเฉลี่ย", array(), array("selectable"=>TRUE,"default"=>TRUE,"class"=>"right","order"=>22));
		$this->_setController("ea_weave_price", "ราคาปักเฉลี่ย", array(), array("selectable"=>TRUE,"default"=>TRUE,"class"=>"right","order"=>23));
		$this->_setController("ea_other_price", "ราคาอื่นๆเฉลี่ย", array(), array("selectable"=>TRUE,"default"=>TRUE,"class"=>"right","order"=>24));
		$this->_setController("total_price_each", "ราคารวมเฉลี่ย", array(), array("selectable"=>TRUE,"default"=>TRUE,"class"=>"right","order"=>25));
*/
		...
		$this->_setController("close_payment_amount", "จำนวนเงินงวดสุดท้าย", array(), array("selectable"=>TRUE,"default"=>TRUE,"class"=>"default_number edit close_payment_amount","order"=>32));
		$this->_setController("close_payment_route", "ช่องทางการชำระงวดสุดท้าย", array(), array("selectable"=>TRUE,"default"=>TRUE,"class"=>"center edit close_payment_route_id","order"=>33));
		$this->_setController("close_payment_date", "วันที่ชำระงวดสุดท้าย", array(), array("selectable"=>TRUE,"default"=>TRUE,"class"=>"center edit close_payment_date","order"=>34));
		$this->_setController("close_payment_wht", "WHT จำนวน", array(), array("selectable"=>TRUE,"default"=>TRUE,"class"=>"center edit close_payment_wht","order"=>35));
		$this->_setController("deliver_remark", "หมายเหตุ", array(), array("selectable"=>TRUE,"default"=>TRUE,"class"=>"center edit deliver_remark","order"=>36));
		...
		$pass['title'] = "รายงานรอจัดส่ง";
		
		$_str_closepayment_route_options = json_encode(hlpr_arrListClosePaymentRoute());
		$this->add_js('var _objClosePaymentRouteOptions = ' . $_str_closepayment_route_options . ';', 'custom_init');
...
<table id="tblSearchResult" class="cls-tbl-list">
	<thead>
		<tr>
			...
			<th colspan="5" class="ui-state-default">งวดสุดท้าย</th>
		</tr>
		<tr>
			<th>จำนวนเงิน</th>
...
TBL;
	...
	function commit() {
		...
		if (isset($_arrData) && ($_arrData != FALSE)) {
			...
			if ($this->mcommit->commit($_arrData, 2)) {
