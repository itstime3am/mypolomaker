...
class Order_premade_other extends MY_Ctrl_crud {
	function __construct() {
		...
		$this->_PRODUCT_TYPE_ID = 12;
		$this->_CUSTOMER_ROWID = -1;
		$this->_START_SCRIPT = '';
	}
	
	public function pass_command($product_type_id = 12, $action = 0, $rowid = 0, $job_number = '', $customer_rowid = 0) {
		if (is_numeric($product_type_id) && ($product_type_id > 0)) $this->_PRODUCT_TYPE_ID = $product_type_id;
	...
	public function index($product_type_id = 12, $customer_rowid = -1) {
		if (is_numeric($product_type_id) && ($product_type_id > 0)) $this->_PRODUCT_TYPE_ID = $product_type_id;
		if (is_numeric($customer_rowid) && ($customer_rowid > 0)) $this->_CUSTOMER_ROWID = $customer_rowid;
		...
		$this->_selOptions = $this->_prepareSelectOptions($_arrPrepare);
		...
		$_arrDtlTabs = hlpr_get_OrderPremadeOther_ViewParams();
		...
		$pass['work_panel'] = $this->add_view('_public/_list', array(
			...
			'edit_template' => $this->load->view('order/form_polo'
				, array(
					"index" => 2
		...
		$pass['title'] = "สั่งสำเร็จรูป:: อื่นๆ";
